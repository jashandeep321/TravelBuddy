...
function NatureDes() {
    const [natureDestinations, setNatureDestinations] = useState([]);
    ...
    useEffect(() => {
        const fetchNatureData = async () => {
            try {
                ...
                const filtered = response.data.filter(dest => dest.category.toLowerCase() === 'nature');
                setNatureDestinations(filtered);
                setLoading(false);
            } catch (err) {
                console.error(err);
